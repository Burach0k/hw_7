...
def join_files(file_path_list):
    ...
    data = create_result_file_data(
        sort_data_list(file_path_list)
    )
...
def create_result_file_data(files_data_list):
    return '\n'.join(
        list(map(child_file_info_map, files_data_list))
    )

def sort_data_list(file_path_list):
    return sorted(get_file_data_list(file_path_list), key=lambda file_data: file_data['line_count'])
